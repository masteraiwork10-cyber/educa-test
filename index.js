...
app.get('/', async (req, res) => {
    try {
        ...
        <body class="bg-slate-50 text-slate-900 flex flex-col min-h-screen">
            ...
            <header class="max-w-7xl mx-auto px-5 py-16 md:py-24 text-center md:text-left grid md:grid-cols-2 gap-12 items-center flex-grow">
                <div><h1 class="text-4xl md:text-5xl font-black mb-6">Master <span class="text-indigo-600">Cloud Engineering.</span></h1><p class="text-slate-500 mb-10">Premium courses hosted on Render. Dedicated to Stephen.</p>
            ...
            <footer class="bg-white border-t border-slate-100 py-12 mt-20">
                <div class="max-w-7xl mx-auto px-5 flex flex-col md:flex-row justify-between items-center gap-6">
                    <div><span class="font-black italic text-indigo-600">EDUCA.</span><p class="text-[10px] text-slate-400 uppercase tracking-widest mt-2">Powered by Render & MongoDB • Designed for Stephen</p></div>
                    <div class="flex gap-8 text-[10px] font-bold uppercase tracking-widest text-slate-400">
                        <a href="/dashboard" class="hover:text-indigo-600">Admin Panel</a>
...
// 2. FIXED STUDENT LOGIN POST ROUTE
    ...
    try {
        ...
        let content = student.enrolledCourses.map(c => `
            <div class="bg-white p-8 rounded-3xl mb-4 shadow-lg border border-slate-50">
                ...
                <div class="flex justify-between items-center"><span class="text-[10px] font-bold text-slate-400 uppercase">${student.courseProgress}% Done</span><a href="/view-certificate/${student.fullName}/${c.title}" class="text-[10px] font-bold text-indigo-600 uppercase border-b border-indigo-600">Certificate</a></div>
            </div>`).join('');
        res.send(`<head><meta name="viewport" content="width=device-width, initial-scale=1.0"><script src="https://cdn.tailwindcss.com"></script></head><body class="bg-slate-50 p-6 md:p-12"><div class="max-w-xl mx-auto"><h1 class="text-2xl font-black mb-8 italic">Welcome, ${student.fullName}</h1>${content}<a href="/" class="block text-center mt-12 text-slate-400 text-xs font-bold uppercase tracking-widest">Logout</a></div></body>`);
    } catch (e) { res.status(500).send(e.message); }
});

// 3. FIXED ADMIN DASHBOARD (With Real Buttons)
app.get('/dashboard', async (req, res) => {
    const token = req.cookies.token;
    if (!token) return res.send("<h1>Unauthorized</h1><a href='/login-demo'>Login as Admin</a>");
    try {
            ...
            <tr class="border-b text-sm">
                ...
                <td class="p-4 text-center">
                    <form action="/update-progress/${s._id}" method="POST" class="flex gap-2 justify-center">
                        ...
                        <button class="bg-slate-800 text-white px-2 py-1 rounded text-[10px] font-bold uppercase">Set</button>
                    </form>
                </td>
                <td class="p-4">
                    <div class="flex gap-2 justify-center">
                        <a href="/download-invoice/${s._id}" class="bg-green-100 text-green-700 px-3 py-1.5 rounded-lg font-bold text-[10px] uppercase">Invoice</a>
                        <a href="/enroll-student/${s._id}" class="bg-indigo-100 text-indigo-700 px-3 py-1.5 rounded-lg font-bold text-[10px] uppercase">Enroll</a>
                        <a href="/delete-student/${s._id}" class="bg-red-100 text-red-700 px-3 py-1.5 rounded-lg font-bold text-[10px] uppercase">Delete</a>
                    </div>
                </td>
            </tr>`).join('');
        ...
        <body class="bg-slate-100 p-4 md:p-8"><div class="max-w-5xl mx-auto bg-white rounded-3xl shadow-xl overflow-hidden">
            <div class="bg-indigo-600 p-6 text-white flex justify-between items-center"><h1 class="text-lg font-black italic text-white">ADMIN PANEL</h1><a href="/" class="text-xs bg-white text-indigo-600 px-4 py-2 rounded-xl font-bold">Exit</a></div>
            <div class="overflow-x-auto"><table class="w-full text-left">
                <thead class="bg-slate-50 text-[10px] font-black text-slate-400 uppercase tracking-widest"><tr><th class="p-4">Student</th><th class="p-4 text-center">Progress %</th><th class="p-4 text-center">Manage</th></tr></thead>
                <tbody>${rows}</tbody>
            </table></div>
        </div></body>`);
...
// [All previous utility routes: /register, /register-student, /add-sample-course, /enroll-student, /delete-student, /login-demo, /download-invoice, /update-progress, /view-certificate]
// Ensure you have /register and /register-student from the previous version!

app.get('/register', (req, res) => {
    res.send(`<head><meta name="viewport" content="width=device-width, initial-scale=1.0"><script src="https://cdn.tailwindcss.com"></script></head><body class="bg-indigo-600 flex items-center justify-center min-h-screen p-6"><form action="/register-student" method="POST" class="bg-white p-8 rounded-[40px] shadow-2xl w-full max-w-md"><h2 class="text-2xl font-black mb-1">Create Account</h2><p class="text-slate-400 text-xs mb-8 font-bold uppercase">Join the Academy</p><input type="text" name="fullName" placeholder="Full Name" class="w-full p-4 mb-3 bg-slate-50 rounded-2xl outline-none" required><input type="email" name="email" placeholder="Email" class="w-full p-4 mb-3 bg-slate-50 rounded-2xl outline-none" required><input type="password" name="password" placeholder="Password" class="w-full p-4 mb-8 bg-slate-50 rounded-2xl outline-none" required><button class="w-full py-4 bg-indigo-600 text-white font-black rounded-2xl uppercase text-xs">Register</button><a href="/" class="block text-center mt-6 text-slate-400 text-[10px] font-black uppercase italic">Home</a></form></body>`);
});

app.post('/register-student', async (req, res) => {
    try { await new User(req.body).save(); res.send("Registered! <a href='/'>Login</a>"); } catch (e) { res.send(e.message); }
});
