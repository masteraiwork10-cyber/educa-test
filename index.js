...
const nodemailer = require('nodemailer'); // NEW: for email notifications
...
const FOOTER = `
<footer class="bg-white border-t border-slate-100 py-14 mt-auto">
    <div class="max-w-7xl mx-auto px-6 flex flex-col md:flex-row justify-between items-center gap-8">
        <div class="flex items-center gap-3">
            ...
            <div>
                ...
                <p class="text-[10px] text-slate-400 uppercase tracking-widest font-bold">SAP / IT Career Launchpad • Bengaluru</p>
            </div>
        </div>
        <div class="flex flex-wrap gap-6 text-[11px] font-black uppercase tracking-widest text-slate-400 justify-center">
            ...
            <p class="text-slate-300">© 2026 EDUCA Academy</p>
        </div>
    </div>
</footer>`;
...
app.get('/', async (req, res) => {
    try {
        ...
        const courseCards = courses.map(c => {
            ...
            return `
            <div class="bg-white rounded-[2.1rem] shadow-sm border border-slate-100 overflow-hidden hover:shadow-2xl hover:-translate-y-2 transition-all duration-500 flex flex-col group">
                <div class="h-44 ${style.bgLetter} flex items-center justify-center overflow-hidden relative">
                ...
                <div class="p-6 flex-grow flex flex-col">
                    ...
                    <h3 class="text-xl font-black text-slate-800 leading-tight mb-1 group-hover:text-indigo-600 transition-colors">${c.title}</h3>
                    <p class="text-[10px] text-indigo-500 font-black uppercase tracking-[0.2em] mb-3">Instructor: ${c.instructor}</p>
                    <p class="text-sm text-slate-500 leading-relaxed line-clamp-3 mb-5">${c.description}</p>
                    <div class="mt-auto flex items-center justify-between pt-4 border-t border-slate-50">
                        ...
                        <a href="/register" class="bg-indigo-600 text-white text-[11px] font-black px-5 py-2.5 rounded-2xl uppercase tracking-widest hover:bg-black shadow-md">Enroll</a>
                    </div>
                </div>
            </div>`;
        }).join('') || `
            <div class="col-span-full bg-white rounded-3xl border border-dashed border-slate-200 p-10 text-center text-slate-400 text-xs font-black uppercase tracking-[0.3em]">
                No courses added yet. Use "Reset Demo Content".
            </div>`;

        // MATERIAL ACCESS CARDS (static – aap chahein to DB se laa sakte hain)
        const materialCards = `
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
                <div class="bg-white border border-slate-100 rounded-3xl p-4 flex flex-col gap-2 hover:-translate-y-1 hover:shadow-lg transition-all">
                    <p class="text-[10px] font-black uppercase tracking-widest text-emerald-500">SAP FICO</p>
                    <h3 class="text-sm font-bold text-slate-800">Configuration Notes</h3>
                    <p class="text-[11px] text-slate-500">Important IMG paths, GL, tax, AR/AP configuration summaries.</p>
                    <button class="mt-2 text-[11px] font-black text-indigo-600 uppercase tracking-widest">Access</button>
                </div>
                <div class="bg-white border border-slate-100 rounded-3xl p-4 flex flex-col gap-2 hover:-translate-y-1 hover:shadow-lg transition-all">
                    <p class="text-[10px] font-black uppercase tracking-widest text-sky-500">SAP FICO</p>
                    <h3 class="text-sm font-bold text-slate-800">Interview Q&A</h3>
                    <p class="text-[11px] text-slate-500">Top questions on integration, tables, error handling.</p>
                    <button class="mt-2 text-[11px] font-black text-indigo-600 uppercase tracking-widest">Access</button>
                </div>
                <div class="bg-white border border-slate-100 rounded-3xl p-4 flex flex-col gap-2 hover:-translate-y-1 hover:shadow-lg transition-all">
                    <p class="text-[10px] font-black uppercase tracking-widest text-violet-500">Finance</p>
                    <h3 class="text-sm font-bold text-slate-800">GST / TDS Basics</h3>
                    <p class="text-[11px] text-slate-500">Concept notes + examples mapped with SAP postings.</p>
                    <button class="mt-2 text-[11px] font-black text-indigo-600 uppercase tracking-widest">Access</button>
                </div>
                <div class="bg-white border border-slate-100 rounded-3xl p-4 flex flex-col gap-2 hover:-translate-y-1 hover:shadow-lg transition-all">
                    <p class="text-[10px] font-black uppercase tracking-widest text-amber-500">Career</p>
                    <h3 class="text-sm font-bold text-slate-800">Resume + Roadmap</h3>
                    <p class="text-[11px] text-slate-500">Sample CV, skill roadmap for FICO, S/4HANA tips.</p>
                    <button class="mt-2 text-[11px] font-black text-indigo-600 uppercase tracking-widest">Access</button>
                </div>
            </div>
        `;

        // Demo video ID (YouTube) – env se configurable
        const demoVideoId = process.env.DEMO_SAP_FICO_VIDEO_ID || 'dQw4w9WgXcQ';
...
<body class="bg-[#F8FAFC] flex flex-col min-h-screen">
    ...
    <nav class="nav-container bg-white/80 backdrop-blur-xl sticky top-0 z-50 border-b border-slate-100 min-h-[5rem] flex items-center justify-between px-4 md:px-8">
        ...
        <div class="nav-auth-buttons flex gap-2 md:gap-4 flex-wrap">
            <a href="#enquiry" class="bg-emerald-50 text-emerald-700 px-4 md:px-6 py-2.5 md:py-3 rounded-2xl text-[10px] md:text-[11px] font-black uppercase tracking-widest hover:bg-emerald-600 hover:text-white transition-all">Course Enquiry</a>
    ...
    <!-- HERO SECTION (SAP / IT + student) -->
    <header class="max-w-7xl mx-auto px-6 py-10 md:py-20 grid md:grid-cols-2 gap-10 items-center">
        <div>
            <span class="bg-indigo-50 text-indigo-600 px-4 py-2 rounded-full text-[10px] font-black uppercase tracking-widest mb-4 inline-block">SAP FICO • IT Training</span>
            <h1 class="text-4xl md:text-6xl font-black mb-5 leading-[1.1] tracking-tight text-slate-900">
                Launch your <span class="text-indigo-600 underline decoration-indigo-200">SAP Career</span> from Bengaluru.
            </h1>
            <p class="text-slate-500 mb-6 text-sm md:text-base max-w-md leading-relaxed">
            ...
            <div class="flex flex-col sm:flex-row gap-3 mb-6">
                <a href="#demo-video" class="bg-indigo-600 text-white px-6 py-3 rounded-2xl text-[11px] font-black uppercase tracking-widest shadow-lg hover:bg-black transition-all">Watch Demo Class</a>
                <a href="#enquiry" class="bg-white text-indigo-600 border border-indigo-100 px-6 py-3 rounded-2xl text-[11px] font-black uppercase tracking-widest hover:bg-indigo-50 transition-all">Request Free Counselling</a>
            </div>
        ...
        <div class="relative">
            <div class="absolute -inset-4 bg-gradient-to-tr from-indigo-500 to-emerald-500 rounded-[3rem] blur-2xl opacity-20"></div>
            <div class="relative rounded-[2.5rem] overflow-hidden shadow-2xl border border-white">
                <img 
            ...
            <div class="absolute -bottom-5 -right-2 bg-white rounded-3xl shadow-xl px-4 py-3 flex items-center gap-3 border border-slate-100">
                <div class="w-8 h-8 rounded-2xl bg-emerald-50 flex items-center justify-center text-emerald-600 text-xs font-black">SAP</div>
                <div>
                    <p class="text-[10px] font-black uppercase tracking-widest text-slate-400">Live Batch</p>
                    <p class="text-xs font-bold text-slate-800">Weekend & Weekday Slots</p>
                </div>
            </div>
        </div>
    </header>
    ...
    <main class="max-w-7xl mx-auto px-6 pb-16 space-y-16">
        ...
        <section id="material">
            <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-6">
                <div>
                    <h2 class="text-2xl md:text-3xl font-black text-slate-900 uppercase">Access to Material</h2>
                    <p class="text-sm text-slate-500 mt-1">Hand-picked SAP FICO and finance resources for practice and revision.</p>
                </div>
                <span class="text-[10px] font-black uppercase tracking-[0.25em] text-emerald-500">Notes • Q&A • Resume • Roadmap</span>
            </div>
            ${materialCards}
        </section>

        <!-- DEMO SAP FICO VIDEO SECTION -->
        <section id="demo-video" class="pt-4">
            <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-6">
                <div>
                    <h2 class="text-2xl md:text-3xl font-black text-slate-900 uppercase">SAP FICO Demo Class</h2>
                    <p class="text-sm text-slate-500 mt-1">Get a feel of the teaching style, depth, and practical coverage.</p>
                </div>
                <p class="text-[10px] font-black uppercase tracking-[0.25em] text-slate-400">Click to play sample session</p>
            </div>
            <div class="w-full rounded-[2rem] overflow-hidden bg-black shadow-2xl border border-slate-900/10">
        ...
        <!-- COURSE TRACKS / CARDS -->
        <section>
            <div class="flex flex-col md:flex-row justify-between items-start md:items-end mb-6 border-l-4 border-indigo-600 pl-4 md:pl-6 gap-3">
                <div>
                    <h2 class="text-2xl md:text-3xl font-black text-slate-900 uppercase">Available Tracks</h2>
                    <p class="text-sm text-slate-500 mt-1">Choose from cloud, security, frontend and SAP-focused programs.</p>
                </div>
                <a href="/add-sample-course" class="bg-white text-indigo-600 px-4 md:px-6 py-2.5 rounded-2xl text-[11px] font-black uppercase border border-slate-200 hover:bg-indigo-50">↻ Reset Demo Content</a>
            </div>
        ...
        <section id="enquiry" class="bg-white border border-slate-100 rounded-[2.5rem] p-6 md:p-10 shadow-xl">
            <div class="flex flex-col md:flex-row justify-between gap-6 mb-6">
                <div>
                    <h2 class="text-2xl md:text-3xl font-black text-slate-900 uppercase">Course Enquiry / Demo</h2>
                ...
                <div class="flex items-start gap-3 text-xs text-slate-500">
                    <span class="inline-flex items-center justify-center w-8 h-8 rounded-full bg-emerald-50 text-emerald-600 text-sm font-black">✓</span>
                    <p>Instant WhatsApp ping to trainer + email notification when you submit this form.</p>
                </div>
            </div>
            <form action="/enquiry" method="POST" class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <input type="text" name="name" placeholder="Full Name" class="w-full p-4 bg-slate-50 rounded-2xl border border-slate-100 outline-none focus:border-indigo-600 text-sm" required>
                <input type="tel" name="phone" placeholder="WhatsApp Number" class="w-full p-4 bg-slate-50 rounded-2xl border border-slate-100 outline-none focus:border-indigo-600 text-sm" required>
                <input type="email" name="email" placeholder="Email Address" class="w-full p-4 bg-slate-50 rounded-2xl border border-slate-100 outline-none focus:border-indigo-600 text-sm">
                <select name="course" class="w-full p-4 bg-slate-50 rounded-2xl border border-slate-100 outline-none focus:border-indigo-600 text-sm">
                ...
                <textarea name="message" rows="3" placeholder="Your query / preferred timings / experience details" class="md:col-span-2 w-full p-4 bg-slate-50 rounded-2xl border border-slate-100 outline-none focus:border-indigo-600 text-sm"></textarea>
                <div class="md:col-span-2 flex flex-col sm:flex-row gap-3 items-start sm:items-center justify-between mt-2">
                    <button type="submit" class="bg-indigo-600 text-white px-6 py-3 rounded-2xl text-[11px] font-black uppercase tracking-widest shadow-lg hover:bg-black transition-all">
                        Submit & Ping on WhatsApp
                    </button>
                    <a href="https://wa.me/${process.env.OWNER_WHATSAPP || '919999999999'}?text=Hi%20I%20want%20SAP%20FICO%20course%20details" 
                       target="_blank"
                       class="inline-flex items-center gap-2 text-[11px] font-black uppercase tracking-widest text-emerald-600">
...
// --- NEW: ENQUIRY FORM HANDLER (WhatsApp + Email) ---
app.post('/enquiry', async (req, res) => {
    try {
        const { name, phone, email, course, message } = req.body;

        const ownerNumber = process.env.OWNER_WHATSAPP || '919999999999'; // with country code only digits
        const baseText = `New%20course%20enquiry%0AName:%20${encodeURIComponent(name)}%0AWhatsApp:%20${encodeURIComponent(phone)}%0AEmail:%20${encodeURIComponent(email || '')}%0ACourse:%20${encodeURIComponent(course)}%0AMessage:%20${encodeURIComponent(message || '')}`;
        const waLink = `https://wa.me/${ownerNumber}?text=${baseText}`; // wa.me prefilled message pattern [web:21][web:24][web:30]

        // Email to owner
        if (process.env.OWNER_EMAIL && transporter) {
        ...
        // Redirect to WhatsApp link so owner ko instant chat trigger ho
        res.redirect(waLink);
    } catch (e) {
                ...
                <div class="max-w-6xl mx-auto bg-white rounded-[2.5rem] shadow-2xl overflow-hidden border border-white">
                    ...
                    <div class="overflow-x-auto">
                        <table class="w-full text-left">
                            ...
                            <tbody>
                                ${rows || '<tr><td colspan="3" class="p-20 text-center font-bold text-slate-300 uppercase">No students found</td></tr>'}
                            </tbody>
                        </table>
                    </div>
                </div>
    ...
    try {
        ...
        let content = student.enrolledCourses.map(c => `
            <div class="bg-white p-6 md:p-10 rounded-[2.5rem] mb-6 shadow-xl border border-slate-50 transition-all hover:scale-[1.01]">
                ...
                <div class="w-full bg-slate-100 h-3 rounded-full my-8 overflow-hidden">
                    <div style="width:${student.courseProgress}%" class="bg-indigo-600 h-full transition-all duration-1000 shadow-lg shadow-indigo-200"></div>
                </div>
        ...
        <body class="bg-[#F8FAFC] p-4 md:p-20 flex flex-col min-h-screen">
...
app.post('/register-student', async (req, res) => {
    try { 
        ...
        res.send(`<html>${headHTML('Success')}<body class="p-20 text-center font-sans"><h1>Success!</h1><a href="/" class="text-indigo-600 font-bold">Go Login</a></body></html>`); 
    } catch (e) { 
        res.send(e.message); 
    }
});
