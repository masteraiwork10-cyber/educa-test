...
// Professional Head Template with embedded Favicon
...
app.get('/', async (req, res) => {
    try {
        ...
        res.send(`<!DOCTYPE html><html lang="en">${headHTML('Educa Academy | Premium LMS')}
        <body class="bg-[#F8FAFC] text-slate-900 flex flex-col min-h-screen">
            ...
            <header class="max-w-7xl mx-auto px-6 py-16 md:py-28 grid md:grid-cols-2 gap-16 items-center flex-grow">
                <div>
                    ...
                    <p class="text-slate-500 mb-12 text-lg max-w-md leading-relaxed">Master enterprise Cloud Engineering. Industry-standard certifications.</p>
            ...
            <tr class="border-b border-slate-50 hover:bg-slate-50/50 transition-colors">
                ...
                <td class="p-6 text-right">
                    <div class="flex gap-3 justify-end">
                        <a href="/download-invoice/${s._id}" class="bg-emerald-500 text-white px-6 py-3 rounded-xl font-black text-[10px] uppercase tracking-widest shadow-lg shadow-emerald-100">Invoice</a>
                        <a href="/enroll-student/${s._id}" class="bg-indigo-600 text-white px-6 py-3 rounded-xl font-black text-[10px] uppercase tracking-widest shadow-lg shadow-indigo-100">Enroll</a>
                        <a href="/delete-student/${s._id}" class="bg-rose-500 text-white px-6 py-3 rounded-xl font-black text-[10px] uppercase tracking-widest shadow-lg shadow-rose-100">Delete</a>
                    </div>
                </td>
            </tr>`).join('');

        res.send(`<!DOCTYPE html><html lang="en">${headHTML('Admin Dashboard')}
        <body class="bg-[#F8FAFC] p-4 md:p-12"><div class="max-w-6xl mx-auto space-y-10">
            <div class="bg-white rounded-[2.5rem] shadow-2xl overflow-hidden border border-white">
                <div class="bg-slate-900 p-8 text-white flex justify-between items-center">
                    <div><h1 class="text-xl font-black italic tracking-widest uppercase">Admin Management</h1></div>
                    <a href="/" class="bg-white text-slate-900 px-6 py-3 rounded-2xl font-black text-[11px] uppercase tracking-widest hover:bg-indigo-600 hover:text-white transition-all">← Exit</a>
                </div>
                <div class="overflow-x-auto"><table class="w-full text-left">
                    ...
                    <tbody>${studentRows || '<tr><td colspan="3" class="p-20 text-center font-bold text-slate-300">NO STUDENTS</td></tr>'}</tbody>
                </table></div>
            </div>
        </div></body></html>`);
    ...
    try {
        ...
        let content = student.enrolledCourses.map(c => `
            <div class="bg-white p-10 rounded-[2.5rem] mb-6 shadow-xl border border-slate-50">
                ...
                <div class="w-full bg-slate-100 h-3 rounded-full my-8 overflow-hidden"><div style="width:${student.courseProgress}%" class="bg-indigo-600 h-full"></div></div>
                <div class="flex justify-between items-center">
                    <a href="/view-lesson/${c._id}" class="bg-indigo-600 text-white px-8 py-4 rounded-2xl text-[11px] font-black uppercase tracking-widest shadow-lg shadow-indigo-100">Resume</a>
                    <a href="/view-certificate/${student.fullName}/${c.title}" class="text-[11px] font-black text-slate-400 uppercase">Certificate</a>
                </div>
            </div>`).join('');
        res.send(`<!DOCTYPE html><html lang="en">${headHTML('Student Portal')}
        <body class="bg-[#F8FAFC] p-6 md:p-20"><div class="max-w-2xl mx-auto">
            <div class="flex justify-between items-center mb-12">
                ...
                <a href="/" class="text-[11px] font-black uppercase text-slate-400">Logout</a>
            </div>
            ${content || '<div class="bg-white p-20 rounded-[2.5rem] text-center">No Courses</div>'}
        </div></body></html>`);
...
app.get('/view-lesson/:courseId', async (req, res) => {
    try {
        ...
        res.send(`<!DOCTYPE html><html lang="en">${headHTML('Learning Console')}
        <body class="bg-slate-950 text-white p-5 md:p-10 flex items-center justify-center min-h-screen"><div class="max-w-5xl w-full">
            <div class="aspect-video w-full rounded-[2.5rem] overflow-hidden bg-black shadow-2xl">
...
app.get('/register', (req, res) => {
    res.send(`<!DOCTYPE html><html lang="en">${headHTML('Join Academy')}
    <body class="bg-indigo-600 flex flex-col items-center justify-center min-h-screen p-6">
        <form action="/register-student" method="POST" class="bg-white p-10 md:p-16 rounded-[3rem] shadow-2xl w-full max-w-md">
            <h2 class="text-3xl font-black mb-10 text-slate-900">Get Started.</h2>
            <input type="text" name="fullName" placeholder="Full Name" class="w-full p-5 mb-4 bg-slate-50 rounded-2xl border border-slate-100 outline-none" required>
            <input type="email" name="email" placeholder="Email" class="w-full p-5 mb-4 bg-slate-50 rounded-2xl border border-slate-100 outline-none" required>
            <input type="password" name="password" placeholder="Password" class="w-full p-5 mb-10 bg-slate-50 rounded-2xl border border-slate-100 outline-none" required>
            <button class="w-full py-5 bg-indigo-600 text-white font-black rounded-2xl uppercase text-[11px] tracking-widest shadow-xl shadow-indigo-100">Create Account</button>
        </form></body></html>`);
});
...
app.get('/add-sample-course', async (req, res) => {
    try {
        ...
        await Course.insertMany([
          { title: "Cloud Engineering with Node.js", instructor: "Stephen", price: 450, tag: "Backend", level: "Advanced", description: "Master scalable backend architecture." },
          { title: "Full-Stack DevSecOps & Security", instructor: "Alex Rivera", price: 550, tag: "Security", level: "Professional", description: "Secure your MERN applications." },
          { title: "Modern UI Design with Tailwind CSS", instructor: "Sarah Chen", price: 350, tag: "Frontend", level: "Intermediate", description: "Build stunning, responsive interfaces." }
        ]);
        res.redirect('/');
    } catch (e) { res.redirect('/'); }
});
...
app.get('/view-certificate/:name/:course', (req, res) => {
    res.send(`<!DOCTYPE html><html lang="en">${headHTML('Certificate of Completion')}
